...
Settings.prototype = {
	...
	update: function( settings, root ) {
		...
		if ( settings.reset === true ) {
			...
			// Re-apply options that were passed into Nodelint constructor
			// but be careful of recursive loop
			if ( settings !== self.nodelint.options ) {
				self.update( self.nodelint.options );
			}
		}
	...
	_addLinters: function( paths, root ) {
		if ( ! Nodelint.isArray( paths ) ) {
			paths = [ paths ];
		}

		// Move nodelint into the global space for use
		var _Nodelint = global.Nodelint;
		global.Nodelint = Nodelint;

		// Require each additional linter
		paths.forEach(function( path ) {
			if ( ! rhome.exec( path ) && ! rpathprefix.exec( path ) ) {
				path = root + '/' + path;
			}

			// Normalize the path
			require( Nodelint.normalizePath( path ) );
		});

		// Remove nodelint from the global space
	...
	_listMaker: function( name, rules, root ) {
		...
		rules.forEach(function( object ) {
			...
			if ( Nodelint.isString( match ) ) {
				if ( ! rhome.exec( match ) && ! rpathprefix.exec( match ) ) {
					match = root + '/' + match;
				}

				// Normalize the path
				match = Nodelint.normalizePath( match );
