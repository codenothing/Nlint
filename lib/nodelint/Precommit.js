...
Nodelint.Precommit = function( type ) {
	if ( type == 'git' ) {
		...
		// Run JSlint on changed file stack
		git.on( 'exit', function( e ) {
			...
			Nodelint( stack, Nodelint.Precommit.Results );
		});
	}
	else if ( type == 'svn' ) {
...
Nodelint.Precommit.All = function(){
	// Set the arguments
	var Files = Nodelint.ARGV();

	// If no files were passed, then assume the whole project needs to be linted
	if ( ! Files.length ) {
		Files = [ process.cwd() ];
	}
...
Nodelint.Precommit.Results = function( e, results ) {
	if ( e ) {
