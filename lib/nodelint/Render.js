...
function Render( files, options, callback ) {
	...
	self.linters = {
		js: require( self.options.jslint || __dirname + '/jslint' ),
		json: require( self.options.jsonlint || __dirname + '/jsonlint' )
	};
	self.ignore = [];
	self._rignore = [];
	self._lintignore = [];
	...
	file: function( file, stat, callback ) {
		...
		self.normalize( file, function( e, file ) {
			...
			fs.readFile( file, 'utf-8', function( e, data ) {
				var info = {}, error = false, ext, linter;
				...
				if ( self.options.encodings ) {
					info = { path: file, stat: stat, content: data, buffer: new Buffer( data ) };
					error = false;

					// Check each encoding for possible conversions
				...
				// Need the extension for linter
				ext = file.split('.').pop();

				// lint the file info
				if ( ext === 'js' ) {
					linter = self.linters.js;
					if ( ! linter( data, self.options._jslint ) ) {
						...
						// Mark count
						self.count.errors += linter.errors.length;
						self.errors.push({
							file: file,
							type: 'js',
							errors: linter.errors.slice( 0 )
						});
					}
					else {
					...
					Nodelint.warn( 'No linter found for ' + ext );
				}
	...
	renderdir: function( path, files, callback ) {
		...
		files.forEach(function( file ) {
			...
			self.stat( file, function( e, stat, file ) {
				...
				// Ensure that it matches our subset of lintable files
				else if ( rlint.exec( file ) ) {
