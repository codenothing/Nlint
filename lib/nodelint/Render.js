...
Render.prototype = {
	...
	fin: function( callback ) {
		callback.call( this, null, this, this.passes, this.errors );
	},
	...
	file: function( file, callback ) {
		...
		self.normalize( file, function( file ) {
			self.readFile( file, function( file, data ) {
				...
				if ( ! jslint.JSLINT( data, self.options.jslint ) ) {
					self.count.errors += jslint.JSLINT.errors.length;
					self.errors.push({
						file: file,
						errors: jslint.JSLINT.errors.slice( 0 )
					});
				}
				else {
