...
Nodelint.ARGV = function( args ) {
	...
	var ARGV = args && Array.isArray( args ) ? args.slice( 0 ) : process.argv.slice( 0 ), Files = [], copy = { nodelint: {}, jslint: {} };

	// Assume binfile alias when not using node initializer
	if ( ( ! args || ! Array.isArray( args ) ) && ARGV.shift() == 'node' ) {
		ARGV.shift();
	}
	...
	return { files: Files, options: copy };
};
